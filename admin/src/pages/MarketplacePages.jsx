...
import { ShoppingBag, Eye, MessageCircle, Star, Edit, Trash2, CheckCircle, XCircle, RefreshCw, Plus, X, AlertTriangle, Tag, Package, Flame, MapPin, Clock } from "lucide-react";
...
export function MarketplaceVenues({ onNavigate }) {
  ...
  const columns = [
    {
      ...
      render: (v, row) => (
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 44, height: 44, borderRadius: 8, overflow: "hidden", background: "#f1f5f9", flexShrink: 0 }}>
            {row.cover_image ? <img src={row.cover_image} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : <Package size={18} style={{ color: "#94a3b8", margin: "13px auto", display: "block" }} />}
          </div>
          <div>
            <div style={{ fontWeight: 600, fontSize: 13 }}>{v}</div>
  ...
  return (
    <div>
      ...
      {loading ? <div style={{ padding: 60, textAlign: "center", color: "#94a3b8" }}>Loading...</div> :
        venues.length === 0
          ? <div style={{ padding: 60, textAlign: "center", background: "#f8fafc", borderRadius: 12, border: "1px solid #e2e8f0" }}><Package size={32} style={{ color: "#e2e8f0", marginBottom: 8, display: "block", margin: "0 auto 8px" }} /><p style={{ color: "#94a3b8", margin: 0 }}>No venues registered yet</p></div>
